 ...
 */
export const namesPlugin: Plugin = (ColordClass, parsers): void => {
  ...
  const NAME_RGBA_STORE: Record<string, RgbColor> = {};

  // Finds a distance between two colors
  // See https://www.wikiwand.com/en/Color_difference
  const getDistanceBetween = (rgb1: RgbColor, rgb2: RgbColor) => {
  ...
  const parseColorName: ParseFunction<string> = (input: string): RgbColor | null => {
