...
export const minifyPlugin: Plugin = (ColordClass): void => {
  ...
  ColordClass.prototype.minify = function minify(options: MinificationOptions = {}) {
    ...
    if (settings.rgb) {
      variants.push(a === 1 ? `rgb(${r},${g},${b})` : `rgb(${r},${g},${b},${a})`);
    }

    // hsl() functional notation with no spaces
    if (settings.hsl) {
      variants.push(a === 1 ? `hsl(${h},${s}%,${l}%)` : `hsl(${h},${s}%,${l}%,${a})`);
    }
